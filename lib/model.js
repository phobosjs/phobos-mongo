...
class PhobosMongoModel {
  ...
  save() {
    ...
    if (this._canonical._id) {
      queryObject.type = 'findOneAndUpdate'
      queryObject.where = { _id: this._canonical._id }
      queryObject.set = Object.assign({}, this._dirty)
    } else {
      queryObject.type = 'insert'
      queryObject.where = Object.assign({}, this._canonical, this._dirty)
    }
  ...
  /**
   * Deletes the doc from the store.
   * @return {Promise} Returns a Promise for the query to delete.
   */
  delete() {
    // We cannot delete something that hasn't yet been saved!
    if (!this._canonical._id) return PhobosMongoModel.runQuery()

    const queryObject = {
      type: 'deleteOne',
      where: { _id: this._canonical._id }
    }
   ...
   * @param {string} query - The query we wish to run
   * @param {array} options - An object with a number of options to govern the returned value
   * @return {Promise} Returns a Promise for the query to save.
   */
  static runQuery(query, { stream = false, lean = false, first = false, last = true } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // We use .apply here because of an issue in the Mongodb driver and its `this`
      run = run[query.type || 'find'].apply(run, query.set ? [query.where, { $set: query.set }] : [query.where])
      ...
      run.then(result => {
        ...
        if (query.type === 'deleteOne' && result && result.deletedCount) {
          result = result.deletedCount === 1
        }

        if (lean || result === null || typeof result === 'boolean') return resolve(result)
